...
contract UserManagement {
    ...
    function register(uint _uniqueId, string memory _name, string memory _fingerprintHash) public {
        require(_uniqueId!=0, "Aadhaar Id not allowed");
    ...
    function transfer(uint _recipientId, string memory _fingerprintHash) payable public onlyRegisteredAddress(msg.sender) onlyRegisteredUser(_recipientId) {
        ...
        require(keccak256(abi.encode(users[senderId].fingerprintHash)) == keccak256(abi.encode(_fingerprintHash)), "Fingerprint Verification Failed");
        // require(users[senderId].fingerprintHash == _fingerprintHash, "Fingerprint verification failed"); // Verify fingerprint
